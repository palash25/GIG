...
class GIG(object):
    ...
    def generate(self, arg):
        if(self.git_not_init()):
            click.echo(click.style("[WARN] Git is not initialized yet",fg='yellow'))
            if(click.confirm('Do you want to initialize GIT in this directory?', abort=False)):
                os.system('git init .')
            else:
                click.echo(click.style("[INFO] GITIGNORE was not generated.",fg='blue'))
                quit()

        if(self.gi_not_exists(self.__dest)):
            self.process(arg)
        else:
            click.echo(click.style("[WARN] GITINGNORE already exists.",fg='yellow'))
